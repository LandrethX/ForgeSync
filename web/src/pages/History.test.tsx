...
const events: HistoryEvent[] = [
  {
    ...
    category: "node",
    actor: "forgesync",
    action: "node.state_changed",
    target: "dk",
    details: { from: "HEALTHY", to: "UNREACHABLE", error: "refused" },
  },
  {
    id: "a3",
  ...
  {
    id: "a2",
    at: "2026-09-18T10:03:00Z",
    category: "conflict",
    actor: "sceneid:bob",
    action: "conflict.acknowledged",
    target: "alice/demo",
    details: { conflict_id: 7, note: "on it" },
  },
];
...
describe("event text", () => {
  it("describes known actions and links to their targets", () => {
    ...
    expect(describeEvent(events[1]!)).toBe(
      "Primary changed from not set to se",
    );
    expect(describeEvent(events[2]!)).toBe("Acknowledged a conflict: “on it”");
    expect(describeEvent({ ...events[0]!, action: "future.thing" })).toBe(
      "future.thing",
    );
    expect(targetLink(events[0]!)).toBe("/nodes/dk");
    expect(targetLink(events[1]!)).toBe(
      "/repositories/11111111-1111-1111-1111-111111111111",
    );
...
function mockApi() {
  const fn = vi.fn(async (url: string) => {
    ...
    if (u.pathname === "/api/v1/history/actors")
      return new Response(JSON.stringify(["forgesync", "sceneid:alice"]));
...
const historyCalls = (fetch: ReturnType<typeof mockApi>) =>
  fetch.mock.calls
    .map(([u]) => new URL(u as string, "http://x"))
    .filter((u) => u.pathname === "/api/v1/history");
...
describe("History page", () => {
  it("lists events with readable text and links", async () => {
    mockApi();
    render(<History />);
    const row = (await screen.findByText("Healthy → Unreachable")).closest(
      "tr",
    )!;
    expect(
      within(row).getByRole("link", { name: "dk" }).getAttribute("href"),
    ).toBe("/nodes/dk");
    expect(
      screen
        .getAllByRole("link", { name: "alice/demo" })[0]!
        .getAttribute("href"),
    ).toBe("/repositories/11111111-1111-1111-1111-111111111111");
  });
  ...
  it("sends filters to the API and keeps them in the address bar", async () => {
    ...
    expect(window.location.search).toBe(
      "?category=session&actor=sceneid%3Aalice",
    );
    const csv = screen
      .getByRole("link", { name: "Export CSV" })
      .getAttribute("href")!;
    expect(csv).toContain(
      "/api/v1/history/export?category=session&actor=sceneid%3Aalice",
    );
